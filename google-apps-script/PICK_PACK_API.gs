...
function ppStaffUpsert_(auth,body){
  ...
  ppMarkMasterMutation_(eventId);const rev=ppBumpRevision_();const master=ppBumpMasterRevision_();const bridge=ppBaoHangBridgeNotifyServiceStaffMutation_(eventId,row,oldCode,'UPSERT');return {ok:true,result:{event_id:eventId,revision:rev,master_revision:master,bao_hang_bridge:bridge}};
}
function ppStaffDelete_(auth,body){
  if(!ppIsAdmin_(auth))return {ok:false,error:'FORBIDDEN'};const eventId=String(body.event_id||'').trim(),mnv=String(body.mnv||'').trim();if(!eventId||!mnv)return {ok:false,error:'STAFF_FIELDS_INVALID'};if(ppMasterMutationSeen_(eventId))return {ok:true,idempotent:true};const active=ppSessionMap_(ppBusinessVisible_())[mnv];if(active&&active.state==='ACTIVE')return {ok:false,error:'STAFF_ACTIVE_SESSION'};const sh=ppSheet_(PP.STAFF),vals=sh.getDataRange().getDisplayValues();let row=0;for(let i=1;i<vals.length;i++){if(String(vals[i][0]||'').trim()===mnv){row=i+1;break;}}if(!row)return {ok:false,error:'EMPLOYEE_NOT_FOUND'};sh.deleteRow(row);ppMarkMasterMutation_(eventId);const rev=ppBumpRevision_();const master=ppBumpMasterRevision_();const bridge=ppBaoHangBridgeNotifyServiceStaffMutation_(eventId,row,mnv,'DELETE');return {ok:true,result:{event_id:eventId,revision:rev,master_revision:master,bao_hang_bridge:bridge}};
}
