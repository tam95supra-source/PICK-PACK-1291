...
class OperationsActivity : Activity() {
    ...
    private fun resourceHome(){
        val root=baseRoot("TÀI NGUYÊN");val body=body();body.addView(txt("Đổi vị trí/PDA/User Pick/Bàn Pack theo transaction atomic. Conflict không làm mất tài nguyên cũ.",10.5f,muted,false));body.addView(gap(9));val mnv=input("MNV",false).apply{setSingleLine(true);imeOptions=EditorInfo.IME_ACTION_DONE;setText(initialMnv)};body.addView(labelled("Mã nhân viên",mnv));body.addView(gap(9));val button=primary("KIỂM TRA PHIÊN",navy){}
    ...
    private fun settingsScreen(){
        val root=baseRoot("CÀI ĐẶT");val body=body();body.addView(section("Tài khoản"));body.addView(listCard("$name • ${roleText(role)}",login));body.addView(gap(7));body.addView(primary("ĐỔI MẬT KHẨU",navy){changePasswordDialog()},matchWrap());if(isAdmin()){body.addView(gap(7));body.addView(primary("QUẢN LÝ TÀI KHOẢN",blue){accountManager()},matchWrap())};body.addView(section("Đồng bộ / dữ liệu"));val sync=info("Đang đọc trạng thái server...");body.addView(sync,matchWrap());api.call("sync_status"){r->runOnUiThread{if(r.ok){val j=r.json?:JSONObject();sync.text="FULL BETA • Server seq ${j.optLong("server_seq")} • chờ Sheet ACK ${j.optInt("projection_pending")} • công nhật đang làm ${j.optInt("labor_active")}"}else sync.text="Không đọc được trạng thái server"}};body.addView(section("Cập nhật"));body.addView(info("${BuildConfig.CHANNEL} • ${BuildConfig.VERSION_NAME}\nTự kiểm tra phiên bản mới: BẬT. App kiểm tra khi mở/foreground và tự hiện thông báo cập nhật."));body.addView(section("Nhật ký"));body.addView(primary("GỬI BÁO LỖI / CHẨN ĐOÁN",teal){sendDiagnostic()},matchWrap());body.addView(section("Thiết bị"));body.addView(info("Android ${Build.VERSION.RELEASE} • ${Build.MANUFACTURER} ${Build.MODEL}"));body.addView(gap(14));body.addView(primary("ĐĂNG XUẤT",red){api.call("logout"){runOnUiThread{finishAffinity()}}},matchWrap());attach(root,body)
    }
