...
class OperationsActivity : Activity() {
    ...
    private fun laborHome() {
        ...
        body.addView(txt("Quét/nhập MNV để bắt đầu hoặc kết thúc công nhật.",10.5f,muted,false));body.addView(gap(9))
        val mnv=input("MNV",false).apply{setSingleLine(true);imeOptions=EditorInfo.IME_ACTION_DONE;setText(initialMnv)}
        body.addView(labelled("Mã nhân viên",mnv));body.addView(gap(9));val button=primary("KIỂM TRA",navy){}
        fun submit(){val v=mnv.text.toString().trim();if(v.isBlank()){toast("Nhập MNV.");return};button.isEnabled=false;button.text="ĐANG KIỂM TRA...";api.call("employee_context",JSONObject().put("mnv",v)){r->runOnUiThread{button.isEnabled=true;button.text="KIỂM TRA";if(handleAuth(r))return@runOnUiThread;if(!r.ok){showError(r.error?:"Không kiểm tra được MNV");return@runOnUiThread};api.call("master_options",JSONObject().put("mnv",v)){m->runOnUiThread{if(handleAuth(m))return@runOnUiThread;showLaborContext(r.json?:JSONObject(),m.json?:JSONObject())}}}}}
        button.setOnClickListener{submit()};mnv.setOnEditorActionListener{_,id,_->if(id==EditorInfo.IME_ACTION_DONE){submit();true}else false};body.addView(button,matchWrap())
    ...
    private fun showLaborContext(ctx:JSONObject, masters:JSONObject){
        ...
        if(state!="ACTIVE"){body.addView(status(if(state=="ENDED")"MNV ĐÃ HẾT PHIÊN" else "MNV CHƯA VÀO CA",red,Color.rgb(255,238,239)));body.addView(gap(9));body.addView(info("Công nhật chỉ thao tác khi nhân sự đang có phiên ACTIVE."));body.addView(gap(10));body.addView(primary("MNV KHÁC",navy){initialMnv="";laborHome()},matchWrap());attach(root,body);return}
    ...
    private fun resourceHome(){
        val root=baseRoot("TÀI NGUYÊN");val body=body();body.addView(txt("Đổi vị trí/PDA/User Pick/Bàn Pack theo transaction khóa Google Sheet. Conflict không làm mất tài nguyên cũ.",10.5f,muted,false));body.addView(gap(9));val mnv=input("MNV",false).apply{setSingleLine(true);imeOptions=EditorInfo.IME_ACTION_DONE;setText(initialMnv)};body.addView(labelled("Mã nhân viên",mnv));body.addView(gap(9));val button=primary("KIỂM TRA PHIÊN",navy){}
        ...
        button.setOnClickListener{submit()};mnv.setOnEditorActionListener{_,id,_->if(id==EditorInfo.IME_ACTION_DONE){submit();true}else false};body.addView(button,matchWrap());if(initialMnv.isNotBlank())button.post{submit()};attach(root,body)
    }
    ...
    private fun listsScreen(){
        ...
        fun searchStaff(){val query=q.text.toString().trim();box.removeAllViews();if(query.length<2){box.addView(info("Nhập ít nhất 2 ký tự để tìm nhân sự."));return};api.call("staff_search",JSONObject().put("query",query)){r->runOnUiThread{box.removeAllViews();if(handleAuth(r))return@runOnUiThread;if(!r.ok){box.addView(info(r.error?:"Lỗi"));return@runOnUiThread};val a=r.json?.optJSONArray("items")?:JSONArray();for(i in 0 until a.length()){val e=a.optJSONObject(i)?:continue;box.addView(listCard("${e.optString("mnv")} • ${e.optString("full_name")}","${e.optString("main_position")} • ${e.optString("supplier")} • ${e.optString("department")}"));box.addView(gap(6))};if(a.length()==0)box.addView(info("Không có kết quả."))}}}
    ...
    private fun reportScreen(){
        val root=baseRoot("BÁO CÁO");val body=body();val box=column(bg);body.addView(box,matchWrap());box.addView(txt("Đang tổng hợp dữ liệu hôm nay...",10.5f,muted,false));api.call("report_daily"){r->runOnUiThread{box.removeAllViews();if(handleAuth(r))return@runOnUiThread;if(!r.ok){box.addView(info(r.error?:"Không tải được báo cáo"));return@runOnUiThread};val j=r.json?:JSONObject();val s=j.optJSONObject("sessions")?:JSONObject();val l=j.optJSONObject("labor")?:JSONObject();val rs=j.optJSONObject("resources")?:JSONObject();box.addView(metric("PHIÊN HÔM NAY","${s.optInt("total")} tổng • ${s.optInt("active")} đang trong ca • ${s.optInt("ended")} đã ra",blue));box.addView(gap(8));box.addView(metric("CÔNG NHẬT","${l.optInt("total")} tổng • ${l.optInt("active")} đang làm • ${l.optInt("completed")} hoàn thành",green));box.addView(gap(8));box.addView(metric("TÀI NGUYÊN","${rs.optInt("currently_assigned")} đang cấp • User Pick đã dùng ${rs.optInt("user_pick_used_today")} • User Pack đã dùng ${rs.optInt("user_pack_used_today")}",orange));box.addView(gap(12));box.addView(jsonMapCard("THEO CA",s.optJSONObject("by_shift")));box.addView(gap(8));box.addView(jsonMapCard("THEO VỊ TRÍ",s.optJSONObject("by_work")));if(isAdmin()){box.addView(gap(8));box.addView(jsonMapCard("CÔNG NHẬT THEO LOẠI",l.optJSONObject("by_type")))}}};attach(root,body)
    }

    private fun settingsScreen(){
        val root=baseRoot("CÀI ĐẶT");val body=body();body.addView(section("Tài khoản"));body.addView(listCard("$name • ${roleText(role)}",login));body.addView(gap(7));body.addView(primary("ĐỔI MẬT KHẨU",navy){changePasswordDialog()},matchWrap());if(isAdmin()){body.addView(gap(7));body.addView(primary("QUẢN LÝ TÀI KHOẢN",blue){accountManager()},matchWrap())};body.addView(section("Đồng bộ / dữ liệu"));val sync=info("Đang đọc trạng thái Google Sheet...");body.addView(sync,matchWrap());api.call("sync_status"){r->runOnUiThread{if(r.ok){val j=r.json?:JSONObject();sync.text="FULL BETA • Google Sheet rev ${j.optLong("server_seq")} • công nhật đang làm ${j.optInt("labor_active")}"}else sync.text="Không đọc được trạng thái Google Sheet"}};body.addView(section("Cập nhật"));body.addView(info("${BuildConfig.CHANNEL} • ${BuildConfig.VERSION_NAME}\nTự kiểm tra phiên bản mới: BẬT. App kiểm tra khi mở/foreground và tự hiện thông báo cập nhật."));body.addView(section("Nhật ký"));body.addView(primary("GỬI BÁO LỖI / CHẨN ĐOÁN",teal){sendDiagnostic()},matchWrap());body.addView(section("Thiết bị"));body.addView(info("Android ${Build.VERSION.RELEASE} • ${Build.MANUFACTURER} ${Build.MODEL}"));body.addView(gap(14));body.addView(primary("ĐĂNG XUẤT",red){api.call("logout"){runOnUiThread{finishAffinity()}}},matchWrap());attach(root,body)
    }
    ...
    private fun sendDiagnostic(){val payload=JSONObject().put("device","${Build.MANUFACTURER} ${Build.MODEL}").put("android",Build.VERSION.RELEASE).put("module",module).put("timestamp",System.currentTimeMillis()).put("channel",BuildConfig.CHANNEL).put("version",BuildConfig.VERSION_NAME);val event=UUID.randomUUID().toString();api.call("diagnostic_log",JSONObject().put("event_id",event).put("channel",BuildConfig.CHANNEL).put("app_version",BuildConfig.VERSION_NAME).put("payload",payload)){r->runOnUiThread{if(handleAuth(r))return@runOnUiThread;if(!r.ok)showError(r.error?:"Không gửi được chẩn đoán")else AlertDialog.Builder(this).setTitle("Đã gửi").setMessage("Google Drive đã ACK diagnostic event $event").setPositiveButton("OK",null).show()}}}
