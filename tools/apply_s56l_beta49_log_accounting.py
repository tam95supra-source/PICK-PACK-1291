...
# Persistent counters survive deletion of successfully uploaded files.
...
for key in ['KEY_GENERATED_BYTES','KEY_SENT_BYTES','KEY_LAST_BYTES','KEY_LAST_AT']:
    if key in s: break
else:
...
# Automatic upload path needs Context threaded through recursion.
...
# Record all generated bytes at write time and keep last-write metadata.
...
required=[MARK,'KEY_GENERATED_BYTES','KEY_SENT_BYTES','recordSent(context,file.length())','recordSent(context,f.length())','uploadNext(context, api, files, 0)']
for x in required:
