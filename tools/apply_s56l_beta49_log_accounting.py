...
if MARK in s and COMPAT in s:
    ...
    runpy.run_path(str(ROOT/'tools/apply_s56m_beta49_compile_context.py'),run_name='__main__')
    raise SystemExit(0)
...
if a<0:
    ...
    line_start=s.rfind('\n',0,a)+1
    if 'S54_BETA48_OWNER_10_FIXES' in s[max(0,line_start-100):a] or 'S56' in s[max(0,line_start-100):a]: a=line_start
...
elif 'private fun recordSent(context:Context,bytes:Long)' not in s: raise SystemExit('S56L write anchor missing')
