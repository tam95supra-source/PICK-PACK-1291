...
checks={
 ...
 'PAYLOAD_SHAPE':"SELECT authority_seq,json_extract(event_json,'$.action') AS action,json_extract(event_json,'$.business_date') AS business_date,json_extract(json_extract(event_json,'$.payload_json'),'$.shift') AS shift,json_extract(json_extract(event_json,'$.payload_json'),'$.work_choice') AS work_choice,json_extract(json_extract(event_json,'$.payload_json'),'$.pda_serial') AS pda_serial,json_extract(json_extract(event_json,'$.payload_json'),'$.user_pick') AS user_pick,json_extract(json_extract(event_json,'$.payload_json'),'$.pack_table') AS pack_table,json_extract(json_extract(event_json,'$.payload_json'),'$.user_pack') AS user_pack,json_extract(json_extract(event_json,'$.payload_json'),'$.labor_type') AS labor_type,json_extract(json_extract(event_json,'$.payload_json'),'$.resource_policy') AS resource_policy FROM fallback_event_inbox WHERE authority_epoch=3 ORDER BY authority_seq",
 'PACK_MAP_COUNTS':"SELECT pack_table,shift,COUNT(*) AS mapping_count FROM resource_pack_map WHERE pack_table IN ('D1','D2') AND shift='Ca 1' AND available=1 GROUP BY pack_table,shift ORDER BY pack_table"
}
