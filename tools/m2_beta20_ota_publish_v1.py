...
ARTIFACT_RUN_ID = os.environ["CANDIDATE_RUN_ID"]
...
def gh_download_candidate():
    dest = Path("/tmp/m2-beta20-candidate")
...
def robust_upload_apk(apk, token_value):
    ...
    upload_id = f"beta20-{os.environ['GITHUB_RUN_ID']}"
...
def validate_owner_trigger():
    trigger = Path("ops/m2-beta20-ota-publish-trigger.txt")
    text = trigger.read_text() if trigger.exists() else ""
    if "confirmation=OWNER_LOCKED_M2_BETA20_OTA_PUBLISH" not in text:
...
publisher.EVIDENCE = Path("m2-beta20-ota-publish-evidence")
publisher.EVIDENCE.mkdir(exist_ok=True)
