...
/**
 * Beta19 fallback rows could carry pack_table but omit user_pack even though the UI displayed the
 * composite mapping. Preserve the immutable/checksummed envelope, then hydrate only the replay
 * payload when D1 proves exactly one active mapping for the same table + shift. Ambiguity is fatal.
 */
async function hydrateLegacyPackUser(db:D1Database,e:FallbackEnvelope,payload:Record<string,unknown>):Promise<void>{
  ...
  if(!shift){
    const mnv=String(payload.mnv||"").trim();
    if(mnv){
      const session=await db.prepare("SELECT shift FROM attendance_sessions WHERE business_date=?1 AND mnv=?2 AND state='ACTIVE'").bind(e.business_date,mnv).first<{shift:string}>();
      shift=String(session?.shift||"").trim();
    }
  }
...
export async function resumeFailbackWithLegacyCompat(db:D1Database,env:Env,input:{fallback_epoch:number;confirmation:string;initiated_by?:string}):Promise<Record<string,unknown>>{
  ...
  await db.prepare("INSERT INTO recovery_runs(recovery_id,recovery_type,from_generation,to_generation,source_authority_epoch,source_authority_seq,target_authority_epoch,status,started_at,validation_json) VALUES(?1,'FAILBACK',?2,?3,?4,?5,?6,'RUNNING',?7,?8)").bind(recoveryId,generation,env.SERVICE_GENERATION,input.fallback_epoch,rows.length,input.fallback_epoch+1,started,JSON.stringify({resume:true,legacy_pack_compat:true,resume_from_seq:before.authority_seq})).run();
  try{
    // The reconciliation lock remains 1, so public mutations stay fenced while replay uses the canonical core.
    ...
    await db.prepare("INSERT INTO authority_transitions(from_epoch,to_epoch,from_mode,to_mode,from_generation,to_generation,reason,initiated_by,checkpoint_epoch,checkpoint_seq,validation_json,created_at) VALUES(?1,?2,'RECONCILING','SERVICE_PRIMARY',?3,?4,'FAILBACK_RESUME_COMPLETE',?5,?1,?6,?7,?8)").bind(input.fallback_epoch,input.fallback_epoch+1,generation,env.SERVICE_GENERATION,String(input.initiated_by||"M2_RECOVERY_RESUME").slice(0,180),rows.length,JSON.stringify({resume:true,legacy_pack_compat:true,applied,event_count:eventCount?.n??0,checksum_verified:true}),at).run();
    await setLock(db,false);
    const after=await currentAuthority(db),validation={resume:true,legacy_pack_compat:true,inbox_count:rows.length,applied,event_count:eventCount?.n??0,checksum_verified:true,contiguous_sequence:true,final_epoch:after.authority_epoch};
