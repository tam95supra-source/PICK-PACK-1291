...
async function token(env:Env):Promise<string>{const body=new URLSearchParams({client_id:env.GOOGLE_OAUTH_CLIENT_ID,client_secret:env.GOOGLE_OAUTH_CLIENT_SECRET,refresh_token:env.GOOGLE_OAUTH_REFRESH_TOKEN,grant_type:"refresh_token"}),r=await fetch("https://oauth2.googleapis.com/token",{method:"POST",headers:{"content-type":"application/x-www-form-urlencoded"},body}),j=await r.json<GoogleToken>();if(!r.ok||!j.access_token)throw new Error(`GOOGLE_OAUTH:${j.error??r.status}`);return j.access_token;}
async function existingIds(env:Env,t:string):Promise<Set<string>>{const range=`${q("LỊCH SỬ NGHIỆP VỤ")}!K2:K`,r=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.GOOGLE_SOURCE_SHEET_ID)}/values/${encodeURIComponent(range)}?majorDimension=ROWS&valueRenderOption=FORMATTED_VALUE`,{headers:{authorization:`Bearer ${t}`}});if(!r.ok)throw new Error(`BETA47_HISTORY_READ:${r.status}`);const rows=(await r.json<{values?:unknown[][]}>()).values??[];return new Set(rows.map(x=>text(x[0])).filter(Boolean));}
async function append(env:Env,t:string,rows:unknown[][]):Promise<void>{if(!rows.length)return;const range=`${q("LỊCH SỬ NGHIỆP VỤ")}!A:M`,r=await fetch(`https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(env.GOOGLE_SOURCE_SHEET_ID)}/values/${encodeURIComponent(range)}:append?valueInputOption=RAW&insertDataOption=INSERT_ROWS`,{method:"POST",headers:{authorization:`Bearer ${t}`,"content-type":"application/json"},body:JSON.stringify({range,majorDimension:"ROWS",values:rows})});if(!r.ok){const x=await r.text();throw new Error(`BETA47_HISTORY_APPEND:${r.status}:${x.slice(0,220)}`);}}
