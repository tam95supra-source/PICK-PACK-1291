...
export async function failbackFromFallbackInbox(db:D1Database,env:Env,input:{fallback_epoch:number;expected_service_epoch:number;confirmation:string;initiated_by?:string}):Promise<Record<string,unknown>>{
  ...
  const rows=(await db.prepare("SELECT event_id,authority_epoch,authority_seq,service_generation,event_json,checksum,ingest_status FROM fallback_event_inbox WHERE authority_epoch=?1 ORDER BY authority_seq").bind(input.fallback_epoch).all<InboxRow>()).results??[];
  ...
  try{
    ...
    for(const row of rows){
      const e=parseEnvelope(row.event_json);await verifyRow(row,e);
      if(row.ingest_status==="APPLIED"){applied++;continue;}
      ...
      await db.prepare("UPDATE fallback_event_inbox SET ingest_status='APPLIED',applied_at=?1,last_error=NULL WHERE event_id=?2").bind(nowIso(),row.event_id).run();applied++;
    }
