...
async function fallbackIngestFenced(request:Request,env:Env):Promise<Response>{
  ...
  const currentFallback=["GOOGLE_FALLBACK","RECONCILING"].includes(a.mode)&&body.authority_epoch===a.authority_epoch;
  ...
  if(existing){
    if(existing.authority_epoch!==body.authority_epoch||existing.authority_seq!==body.authority_seq||existing.checksum!==checksum)return apiError("FALLBACK_EVENT_ID_COLLISION","INTEGRITY",409);
