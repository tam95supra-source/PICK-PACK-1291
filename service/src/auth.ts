...
export async function createChallenge(db: D1Database, loginId: string): Promise<Record<string,unknown>> {
  ...
  const challengeId=crypto.randomUUID(), challenge=randomB64u(32), fakeSalt=randomB64u(16), expires=Date.now()+120_000;
  await db.prepare("DELETE FROM auth_challenges WHERE expires_at<?1").bind(Date.now()).run();
  await db.prepare("INSERT INTO auth_challenges(challenge_id,login_id,purpose,challenge,expires_at,created_at) VALUES(?1,?2,'LOGIN',?3,?4,?5)").bind(challengeId,loginId,challenge,expires,nowIso()).run();
...
export async function createSession(db: D1Database, env: Env, input: {login_id:string;challenge_id:string;proof:string;device_id:string;device_label?:string}): Promise<Response|Record<string,unknown>> {
  const challenge=await db.prepare("SELECT challenge_id,login_id,challenge,expires_at FROM auth_challenges WHERE challenge_id=?1 AND login_id=?2 AND purpose='LOGIN'").bind(input.challenge_id,input.login_id).first<{challenge_id:string;login_id:string;challenge:string;expires_at:number}>();
  await db.prepare("DELETE FROM auth_challenges WHERE challenge_id=?1").bind(input.challenge_id).run();
  const account=await db.prepare("SELECT login_id,verifier,verifier_hash,role,display_name,position,email,status FROM accounts WHERE login_id=?1").bind(input.login_id).first<AccountRow>();
